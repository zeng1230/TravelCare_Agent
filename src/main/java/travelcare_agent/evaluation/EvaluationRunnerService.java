...
public class EvaluationRunnerService {
    ...
    private EvaluationScoringContext context(EvaluationRun run,EvaluationCase c,DryRunResult dr,TraceQueryService.TraceDetail d,SideEffectCheckResult se)throws Exception{EvaluationScoringContext x=new EvaluationScoringContext();x.run=run;x.evaluationCase=c;x.expectation=json.readTree(c.getExpectationJson());x.sourceTraceId=c.getSourceTraceId();x.dryRunTraceId=traceRuns.findByTraceId(dr.dryRunTraceId()).map(TraceRun::getId).orElse(null);x.diffId=dr.diffId();x.dryRunResult=dr;x.sideEffectCheckResult=se;x.promptStubVersion=run.getPromptStubVersion();x.providerMode=run.getProviderMode();x.clock=clock;x.riskLevel=dr.riskLevel();x.spanTypes=d.spans().stream().map(TraceSpan::getSpanType).distinct().toList();List<String> names=new ArrayList<>();d.events().forEach(e->{names.add(e.getEventType());names.add(e.getName());});d.snapshots().forEach(s->names.add(s.getSnapshotType()));x.eventNames=names;x.policyDecision=snapshotText(d,"POLICY_DECISION","decision");x.workflowStatus=snapshotText(d,"WORKFLOW_PATH","status");x.output=snapshotText(d,"FINAL_OUTPUT","answer");applyStage9Context(x,d);return x;}
    private void applyStage9Context(EvaluationScoringContext x,TraceQueryService.TraceDetail d)throws Exception{JsonNode answerability=snapshotJson(d,"ANSWERABILITY_DECISION"),citation=snapshotJson(d,"CITATION_SUMMARY"),finalOutput=snapshotJson(d,"FINAL_OUTPUT");x.answerabilityDecisionSnapshot=answerability;x.citationSummarySnapshot=citation;if(answerability!=null){x.answerabilityStatus=text(answerability,"status");x.answerabilityReasonCode=text(answerability,"reasonCode");x.requiredAction=text(answerability,"requiredAction");x.businessDecisionLocked=bool(answerability,"businessDecisionLocked");x.ragMayExplainBusinessDecision=bool(answerability,"ragMayExplainBusinessDecision");x.ragMayOverrideBusinessDecision=bool(answerability,"ragMayOverrideBusinessDecision");}if(citation!=null){x.citations=citation.path("citations");x.rejectedCitationCandidates=citation.path("rejectedCitationCandidates");}if(finalOutput!=null&&finalOutput.has("fallbackUsed"))x.fallbackUsed=finalOutput.path("fallbackUsed").asBoolean(false);}
    ...
    private JsonNode snapshotJson(TraceQueryService.TraceDetail d,String type)throws Exception{return d.snapshots().stream().filter(s->type.equals(s.getSnapshotType())).reduce((a,b)->b).map(s->{try{return json.readTree(s.getPayloadJson());}catch(Exception e){return null;}}).orElse(null);}
    private static String text(JsonNode n,String field){JsonNode v=n.path(field);return v.isMissingNode()||v.isNull()?null:v.asText();}
    private static Boolean bool(JsonNode n,String field){JsonNode v=n.path(field);return v.isMissingNode()||v.isNull()?null:v.asBoolean(false);}
