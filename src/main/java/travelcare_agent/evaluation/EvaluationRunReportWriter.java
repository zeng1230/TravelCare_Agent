...
public class EvaluationRunReportWriter {
    ...
    private void appendStage9(StringBuilder b,List<ScoreResult> scores){Map<String,Object> actual=null;for(ScoreResult score:scores){if(score.actual() instanceof Map<?,?> m&&m.containsKey("answerabilityStatus")){actual=(Map<String,Object>)m;break;}}if(actual==null)return;b.append("\n### Answerability / Citation\n\n");line(b,"answerabilityStatus",actual.get("answerabilityStatus"));line(b,"answerabilityReasonCode",actual.get("answerabilityReasonCode"));line(b,"requiredAction",actual.get("requiredAction"));line(b,"fallbackUsed",actual.get("fallbackUsed"));line(b,"businessDecisionLocked",actual.get("businessDecisionLocked"));line(b,"ragMayExplainBusinessDecision",actual.get("ragMayExplainBusinessDecision"));line(b,"ragMayOverrideBusinessDecision",actual.get("ragMayOverrideBusinessDecision"));line(b,"citationChunkIds",actual.get("citationChunkIds"));line(b,"rejectedCitationCandidates",actual.get("rejectedCitationCandidates"));}
}
